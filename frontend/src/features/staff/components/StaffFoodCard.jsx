...
import { useStaffCartStore } from "@/features/staff/store/useStaffCartStore";
import { formatCurrency } from "@/shared/utils/format";

const StaffFoodCard = ({ dish, tableId, activeQuantity = 0 }) => {
  const addToCart = useStaffCartStore(state => state.addToCart);
  ...
  return (
    <div className="bg-white rounded-xl p-1.5 shadow-sm border border-gray-50 flex flex-col gap-1.5 relative overflow-hidden group hover:shadow-md transition-all">
      ...
      {activeQuantity > 0 && (
        <div className="absolute top-1.5 left-1.5 bg-blue-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full z-10 shadow-sm border border-blue-400">
          Đang sử dụng: x{activeQuantity}
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => {
              ...
              toast.success(`${dish.name} đã được thêm vào giỏ`);
            }} 
